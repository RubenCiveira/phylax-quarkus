...
- [x] **F11.1a** Crear `features/oauth/authentication/domain/ChallengesState.java`
...
- [x] **F11.1b** Crear `features/oauth/authentication/infrastructure/driver/html/StepInput.java`
  - `@Value @Builder`
  - Campos: `AuthRequest request`, `ClientDetails clientDetails`,
    `Optional<ChallengesState> challenges`, `MultivaluedMap<String, String> formParams`
  - Helpers delegados: `currentUser()`, `step()`, `locale()`, `tenant()`
  - Nota: usa `AuthRequest` directamente durante la transición

- [x] **F11.1c** Crear `features/oauth/authentication/infrastructure/driver/html/StepOutcome.java`
  ```java
...
- [x] **F11.1d** Eliminar marca `/* @autogenerated */` de `FrontAcessController.java` y de
...
- [x] **F11.2a** Crear `features/oauth/authentication/infrastructure/driver/html/OidcStep.java`
  - Interfaz con `challenge()`, `challengeException()` (default null), `stepNames()`,
    `paint(StepInput, NewCookie)`, `process(StepInput)`

- [x] **F11.2b** Crear `features/oauth/authentication/infrastructure/driver/html/OidcStepRouter.java`
  - `@ApplicationScoped`
  - `@Inject @Any Instance<OidcStep> allSteps`
  - Cache lazy+synchronized: `byStepName` y `byException`
  - `process(StepInput)`, `paint(Class, StepInput, NewCookie)`, `challengeFor(Class)`
  - Log de warning si dos steps declaran el mismo step name o excepción

- [x] **F11.2c** Crear `features/oauth/authentication/infrastructure/driver/html/OidcUrlBuilder.java`
  - `@ApplicationScoped @RequiredArgsConstructor`
  - `issuer(String tenant)`, `recoverCallbackUrl(StepInput, String)`,
    `registerCallbackUrl(StepInput, String)`

- [x] **F11.2d** Añadir en `StepResult` (compatibilidad temporal):
  ```java
...
- [x] Crear `step/MfaStep.java` con `@ApplicationScoped`
  - `challenge()` → `AuthenticationChallege.MFA`
  - `challengeException()` → `MfaRequiredException.class`
  - `stepNames()` → `Set.of("mfa")`
  - `paint()`: construye el formulario MFA con cookie de pre-sesión
  - `process()`: verifica OTP via `UserMfa`, usa `i18n("mfa.wrong-code")`
- [x] Añadir `mfa.wrong-code: "Incorrect MFA code"` a `oauth.yaml`
- [ ] Eliminar `MfaControllerPart.java` — **pendiente hasta F11.4** (router no conectado aún)
