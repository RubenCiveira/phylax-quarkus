...
## Fase 11 — Refactor ControllerParts → OidcStep

> Implementación concreta del diseño definido en [ANALISIS_DISEÑO_FORMS_OIDC.md](ANALISIS_DISEÑO_FORMS_OIDC.md).
> Cubre los puntos T8.1–T8.8 de Fase 8 con nomenclatura definitiva y plan granular.
>
> **Estado:** PENDIENTE
> **Prerrequisito:** Compilación en verde (ya alcanzado en T10.2).

---

### Decisiones de diseño clave

#### D1 — Flujo de ChallengesState a través de StepOutcome

La clase `Challenge` (inner class mutable de `FrontAcessController`) se reemplaza por
`ChallengesState` (immutable). El flujo de actualización de challenges queda:

```
Step completa
  → devuelve StepOutcome.Proceed(username, clientDetails, authRequest, currentChallengesState)

FrontAcessController.handleProceed(proceed):
  → llama fillPreAuthenticated(request, username, clientDetails,
                               proceed.challengesState().getCompleted())
    → si OK      → emite código de autorización, borra cookie PRE_SESSION_ID
    → si nuevo challenge (MfaRequiredException, etc.)
        → ch = router.challengeFor(exception.getClass())
        → next = proceed.challengesState().withCompleted(ch)
        → crea nueva pre-session cookie con `next`
        → router.paint(exception.getClass(), input, preSessionCookie(next, tenant))
```

`StepOutcome.Proceed` porta el estado *antes* de que el controlador añada el nuevo
challenge. El controlador es el único responsable de actualizar `ChallengesState`.
Los steps nunca modifican el estado de challenges.

#### D2 — ChallengesState.completed = challenges YA superados

`completed` contiene los challenges que el usuario **ya pasó**. Se pasan a
`fillPreAuthenticated()` para que el use case decida si quedan más.

Compatibilidad hacia atrás con el campo `challenges` del JSON de `Challenge`:

```java
@JsonAlias("challenges")
private List<AuthenticationChallege> completed = List.of();
```

#### D3 — OidcStep.challengeException() para mapear excepción → step

Además de `challenge()` (el enum `AuthenticationChallege`), cada step declara la clase
de excepción que desencadena su renderizado:

```java
default Class<? extends AuthenticationException> challengeException() { return null; }
```

`OidcStepRouter` usa este método para construir el mapa `byException`.

---

### F11.1 — Crear estructuras de datos nuevas

**Objetivo:** Introducir los nuevos tipos sin tocar el flujo existente.
**Criterio de fin:** Todos los tipos compilan. Tests pasan sin cambios de comportamiento.

- [ ] **F11.1a** Crear `features/oauth/authentication/domain/ChallengesState.java`
  - `@Value @Builder @RegisterForReflection`
  - `String username`
  - `@JsonAlias("challenges") @Builder.Default List<AuthenticationChallege> completed = List.of()`
  - `ChallengesState withCompleted(AuthenticationChallege)` → nuevo estado inmutable
  - `static ChallengesState empty(String username)` → builder con lista vacía

- [ ] **F11.1b** Crear `features/oauth/authentication/infrastructure/driver/html/StepInput.java`
  - `@Value @Builder`
  - Campos: `OidcFlowContext context`, `ClientDetails clientDetails`,
    `Optional<ChallengesState> challenges`, `MultivaluedMap<String, String> formParams`
  - Helpers delegados: `currentUser()`, `step()`, `locale()`, `tenant()`, `toAuthRequest()`
  - Nota: `OidcFlowContext` puede completarse en fases posteriores; durante la transición
    puede construirse desde `AuthRequest` ya existente

- [ ] **F11.1c** Crear `features/oauth/authentication/infrastructure/driver/html/StepOutcome.java`
  ```java
  public sealed interface StepOutcome permits StepOutcome.Render, StepOutcome.Proceed {
      ...
      record Proceed(String username, ClientDetails clientDetails,
                     AuthRequest request, ChallengesState challengesState)
              implements StepOutcome {}
  }
  ```

- [ ] **F11.1d** Eliminar marca `/* @autogenerated */` de `FrontAcessController.java` y de
  todos los ficheros en `part/`

---

### F11.2 — Interfaz OidcStep, OidcStepRouter y OidcUrlBuilder

**Objetivo:** Infraestructura de routing, sin conectar aún al controlador.
**Criterio de fin:** Router compila, inyección CDI sin errores, cero steps registrados es válido.

- [ ] **F11.2a** Crear `features/oauth/authentication/infrastructure/driver/html/OidcStep.java`
  ```java
  public interface OidcStep {
      /** Challenge que este step satisface. Null si no corresponde a challenge. */
      AuthenticationChallege challenge();

      /** Excepción que desencadena el paint de este step. Null si no aplica. */
      default Class<? extends AuthenticationException> challengeException() { return null; }

      /** Nombres de "step=" del form que este step puede manejar. */
      Set<String> stepNames();

      /** Renderiza el formulario. La cookie preSession ya está construida. */
      Response paint(StepInput input, NewCookie preSession);

      /** Procesa el formulario enviado. Empty = este step no aplica al step actual. */
      Optional<StepOutcome> process(StepInput input);
  }
  ```

- [ ] **F11.2b** Crear `features/oauth/authentication/infrastructure/driver/html/OidcStepRouter.java`
  - `@ApplicationScoped`
  - `@Inject @Any Instance<OidcStep> allSteps`
  - Cache inicializado lazy+synchronized: `Map<String, OidcStep> byStepName`,
    `Map<Class<? extends AuthenticationException>, OidcStep> byException`
  - `Optional<StepOutcome> process(StepInput input)`
  - `Optional<Response> paint(Class<? extends AuthenticationException>, StepInput, NewCookie)`
  - `Optional<AuthenticationChallege> challengeFor(Class<? extends AuthenticationException>)`

- [ ] **F11.2c** Crear `features/oauth/authentication/infrastructure/driver/html/OidcUrlBuilder.java`
  - `@ApplicationScoped @RequiredArgsConstructor`
  - Mueve `issuer(String tenant)` de `RecoverControllerPart` y `RegistrationControllerPart`
  - `recoverCallbackUrl(StepInput, String username)` y `registerCallbackUrl(StepInput, String email)`

- [ ] **F11.2d** Añadir en `StepResult` (compatibilidad temporal):
  ```java
  public StepOutcome.Proceed toOutcome(ChallengesState state) {
      return new StepOutcome.Proceed(username, clientDetails, request, state);
  }
  ```

---

### F11.3 — Migrar ControllerParts a OidcStep (uno a uno)

**Patrón por cada migración:**
1. Crear `XxxStep implements OidcStep` con `@ApplicationScoped`
2. `stepNames()` declara todos los step names (incluidos los `show-xxx` del `else if`)
3. Mover `doPaint*` → `paint()`
4. Mover `process()` → `process()`, reemplazar `resolver.apply(StepResult.builder()...build())`
   por `Optional.of(new StepOutcome.Proceed(username, clientDetails, request, input.getChallenges().orElseGet(() -> ChallengesState.empty(username))))`
5. Reemplazar strings hardcodeados con i18n (ver tabla §F11.5)
6. Eliminar la clase `XxxControllerPart` original
7. Correr `mvn test -Dgroups="oidc-flow"` para verificar

**Ubicación de los nuevos steps:** `driver/html/step/`

#### F11.3.1 — MfaStep

- [ ] Crear `step/MfaStep.java` con `@ApplicationScoped`
  - `challenge()` → `AuthenticationChallege.MFA`
  - `challengeException()` → `MfaRequiredException.class`
  - `stepNames()` → `Set.of("mfa")`
  - `paint()`: delega a la lógica de `doPaintMfaForm`
  - `process()`: lógica de `doExecMfa`, reemplaza `"MFA incorrecto"` por
    `i18n(input.locale(), "mfa.wrong-code")`
- [ ] Eliminar `MfaControllerPart.java`
...
### F11.5 — Simplificar FrontAcessController
...
### F11.6 — Eliminar Challenge, completar ChallengesState
...
### Strings i18n a añadir en oauth.yaml (durante F11.3)

| Key | Texto hardcodeado | Step | Fichero actual |
|---|---|---|---|
| `mfa.wrong-code` | `"MFA incorrecto"` | F11.3.1 | `MfaControllerPart` |
| `password.save-error` | `"No se ha podido guardar"` | F11.3.4 | `NewPassControllerPart` |
| `recover.save-error` | `"No se ha podido guardar"` | F11.3.7 | `RecoverControllerPart` |
| `login.wrong-credentials` | `"Credenciales incorrectas"` | F11.4 | `FrontAcessController` |

---

### Ficheros nuevos a crear (Fase 11)

| Fichero | Fase | Capa |
|---|---|---|
| `domain/ChallengesState.java` | F11.1a | domain |
| `driver/html/StepInput.java` | F11.1b | infra |
| `driver/html/StepOutcome.java` | F11.1c | infra |
| `driver/html/OidcStep.java` | F11.2a | infra |
| `driver/html/OidcStepRouter.java` | F11.2b | infra |
| `driver/html/OidcUrlBuilder.java` | F11.2c | infra |
| `driver/html/step/MfaStep.java` | F11.3.1 | infra |
| `driver/html/step/ScopeConsentStep.java` | F11.3.2 | infra |
| `driver/html/step/ConsentStep.java` | F11.3.3 | infra |
| `driver/html/step/NewPassStep.java` | F11.3.4 | infra |
| `driver/html/step/NewMfaStep.java` | F11.3.5 | infra |
| `driver/html/step/DelegatedStep.java` | F11.3.6 | infra |
| `driver/html/step/RecoverStep.java` | F11.3.7 | infra |
| `driver/html/step/RegistrationStep.java` | F11.3.8 | infra |

Ruta base: `src/main/java/net/civeira/phylax/features/oauth/authentication/`

### Ficheros a eliminar (Fase 11)

| Fichero | Fase |
|---|---|
| `part/MfaControllerPart.java` | F11.3.1 |
| `part/ScopeConsentControllerPart.java` | F11.3.2 |
| `part/ConsentControllerPart.java` | F11.3.3 |
| `part/NewPassControllerPart.java` | F11.3.4 |
| `part/NewMfaControllerPart.java` | F11.3.5 |
| `part/DelegatedControllerPart.java` | F11.3.6 |
| `part/RecoverControllerPart.java` | F11.3.7 |
| `part/RegistrationControllerPart.java` | F11.3.8 |
| inner class `FrontAcessController.Challenge` | F11.6 |
| inner class `FrontAcessController.FlowInfo` | F11.5 |
| inner class `FrontAcessController.StartFormFlow` | F11.5 |
| inner class `FrontAcessController.StepResult` | F11.5 |
