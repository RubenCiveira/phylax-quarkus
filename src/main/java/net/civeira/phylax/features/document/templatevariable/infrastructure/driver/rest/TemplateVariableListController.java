...
import net.civeira.phylax.features.document.templatevariable.application.usecase.list.TemplateVariableListCursor;
import net.civeira.phylax.features.document.templatevariable.application.usecase.list.TemplateVariableListFilter;
import net.civeira.phylax.features.document.templatevariable.application.usecase.list.TemplateVariableListOrder;
import net.civeira.phylax.features.document.templatevariable.application.usecase.list.TemplateVariableListProjection;
...
import net.civeira.phylax.generated.openapi.model.TemplateVariableApiDtoList;
import net.civeira.phylax.generated.openapi.model.TenantApiRef;

@RequiredArgsConstructor
@ApplicationScoped
public class TemplateVariableListController {
   ...
   * @return
   */
  @ApiObserved("Api to list an entity of template variable")
  public Response templateVariableApiList(final List<String> uids, final String search,
    ...
    List<TemplateVariableListOrder> orderSteps = null == order ? List.of()
        : Arrays.asList(order.split(",")).stream().map(this::mapOrder).filter(Objects::nonNull)
            .toList();
    TemplateVariableListFilter.TemplateVariableListFilterBuilder filterBuilder =
        TemplateVariableListFilter.builder();
    TemplateVariableListCursor.TemplateVariableListCursorBuilder cursorBuilder =
        TemplateVariableListCursor.builder();
    ...
    TemplateVariableListFilter filter = filterBuilder.build();
    TemplateVariableListCursor cursor = cursorBuilder.build();
    List<TemplateVariableListProjection> listed =
    ...
    return currentRequest.cacheableResponse(listed, toListApiModel(listed, filter, cursor),
        "template-variable-" + ("" + filter + cursor).hashCode());
  }
   ...
   * @return
   */
  private TemplateVariableListOrder mapOrder(final String label) {
    if (null == label) {
      return null;
    } else if (label.trim().equals("code-asc")) {
      return TemplateVariableListOrder.CODE_ASC;
    } else if (label.trim().equals("code-desc")) {
      return TemplateVariableListOrder.CODE_DESC;
    } else {
      return null;
    }
  }
   ...
   * @return
   */
  private TemplateVariableApiDto toApiModel(TemplateVariableListProjection dto) {
   ...
   * @return
   */
  private TemplateVariableApiDtoList toListApiModel(
      List<TemplateVariableListProjection> templateVariables, TemplateVariableListFilter filter,
      TemplateVariableListCursor cursor) {
    Optional<TemplateVariableListProjection> last = templateVariables.isEmpty() ? Optional.empty()
        : Optional.of(templateVariables.get(templateVariables.size() - 1));
    ...
    return new TemplateVariableApiDtoList()
        .items(templateVariables.stream().map(this::toApiModel).toList())
   ...
   * @return
   */
  private String writeOrder(final TemplateVariableListOrder order) {
    if (null == order) {
      return null;
    } else if (order == TemplateVariableListOrder.CODE_ASC) {
      return "code-asc";
    } else if (order == TemplateVariableListOrder.CODE_DESC) {
      return "code-desc";
    } else {
      return null;
    }
