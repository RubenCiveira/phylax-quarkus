...
import net.civeira.phylax.generated.openapi.api.ThemeAssetAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class ThemeAssetAuditController implements ThemeAssetAuditApi {
   ...
   */
  @Override
  public Response themeAssetApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName();
    List<AuditEvent> events = reader.findByFilters("document_theme_asset_audit", "themeAsset",
   ...
   */
  @Override
  public Response themeAssetApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName();
    List<AuditEvent> events = reader.findByFilters("document_theme_asset_audit", "themeAsset",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("themeAsset").entityType(event.getEntityType()).entityId(event.getEntityId())
