...
import net.civeira.phylax.generated.openapi.api.TemplateAssetAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class TemplateAssetAuditController implements TemplateAssetAuditApi {
   ...
   */
  @Override
  public Response templateAssetApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("document_template_asset_audit", "templateAsset",
   ...
   */
  @Override
  public Response templateAssetApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("document_template_asset_audit", "templateAsset",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("templateAsset").entityType(event.getEntityType()).entityId(event.getEntityId())
