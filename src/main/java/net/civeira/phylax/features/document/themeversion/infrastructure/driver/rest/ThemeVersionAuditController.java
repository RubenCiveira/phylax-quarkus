...
import net.civeira.phylax.generated.openapi.api.ThemeVersionAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class ThemeVersionAuditController implements ThemeVersionAuditApi {
   ...
   */
  @Override
  public Response themeVersionApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName();
    List<AuditEvent> events = reader.findByFilters("document_theme_version_audit", "themeVersion",
   ...
   */
  @Override
  public Response themeVersionApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName();
    List<AuditEvent> events = reader.findByFilters("document_theme_version_audit", "themeVersion",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("themeVersion").entityType(event.getEntityType()).entityId(event.getEntityId())
