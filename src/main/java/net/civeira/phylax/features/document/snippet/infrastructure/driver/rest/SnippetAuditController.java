...
import net.civeira.phylax.generated.openapi.api.SnippetAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class SnippetAuditController implements SnippetAuditApi {
   ...
   */
  @Override
  public Response snippetApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName();
    List<AuditEvent> events = reader.findByFilters("document_snippet_audit", "snippet",
   ...
   */
  @Override
  public Response snippetApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName();
    List<AuditEvent> events = reader.findByFilters("document_snippet_audit", "snippet",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("snippet").entityType(event.getEntityType()).entityId(event.getEntityId())
