...
import net.civeira.phylax.generated.openapi.api.SnippetAssetAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class SnippetAssetAuditController implements SnippetAssetAuditApi {
   ...
   */
  @Override
  public Response snippetAssetApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("document_snippet_asset_audit", "snippetAsset",
   ...
   */
  @Override
  public Response snippetAssetApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("document_snippet_asset_audit", "snippetAsset",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("snippetAsset").entityType(event.getEntityType()).entityId(event.getEntityId())
