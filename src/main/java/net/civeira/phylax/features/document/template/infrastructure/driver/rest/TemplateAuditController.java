...
import net.civeira.phylax.generated.openapi.api.TemplateAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class TemplateAuditController implements TemplateAuditApi {
   ...
   */
  @Override
  public Response templateApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("document_template_audit", "template",
   ...
   */
  @Override
  public Response templateApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("document_template_audit", "template",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("template").entityType(event.getEntityType()).entityId(event.getEntityId())
