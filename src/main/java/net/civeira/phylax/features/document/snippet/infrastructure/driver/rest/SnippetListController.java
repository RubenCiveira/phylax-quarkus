...
import net.civeira.phylax.features.document.snippet.application.usecase.list.SnippetListCursor;
import net.civeira.phylax.features.document.snippet.application.usecase.list.SnippetListFilter;
import net.civeira.phylax.features.document.snippet.application.usecase.list.SnippetListOrder;
import net.civeira.phylax.features.document.snippet.application.usecase.list.SnippetListProjection;
...
import net.civeira.phylax.generated.openapi.model.SnippetApiDtoList;
...
public class SnippetListController {
   ...
   * @return
   */
  @ApiObserved("Api to list an entity of snippet")
  public Response snippetApiList(final List<String> uids, final String search, final Boolean global,
    ...
    List<SnippetListOrder> orderSteps = null == order ? List.of()
        : Arrays.asList(order.split(",")).stream().map(this::mapOrder).filter(Objects::nonNull)
            .toList();
    SnippetListFilter.SnippetListFilterBuilder filterBuilder = SnippetListFilter.builder();
    SnippetListCursor.SnippetListCursorBuilder cursorBuilder = SnippetListCursor.builder();
    ...
    SnippetListFilter filter = filterBuilder.build();
    SnippetListCursor cursor = cursorBuilder.build();
    List<SnippetListProjection> listed = list.list(currentRequest.interaction(), filter, cursor);
    return currentRequest.cacheableResponse(listed, toListApiModel(listed, filter, cursor),
        "snippet-" + ("" + filter + cursor).hashCode());
  }
   ...
   * @return
   */
  private SnippetListOrder mapOrder(final String label) {
    if (null == label) {
      return null;
    } else if (label.trim().equals("code-asc")) {
      return SnippetListOrder.CODE_ASC;
    } else if (label.trim().equals("code-desc")) {
      return SnippetListOrder.CODE_DESC;
    } else {
      return null;
    }
  }
   ...
   * @return
   */
  private SnippetApiDto toApiModel(SnippetListProjection dto) {
   ...
   * @return
   */
  private SnippetApiDtoList toListApiModel(List<SnippetListProjection> snippets,
      SnippetListFilter filter, SnippetListCursor cursor) {
    Optional<SnippetListProjection> last =
        snippets.isEmpty() ? Optional.empty() : Optional.of(snippets.get(snippets.size() - 1));
    ...
    return new SnippetApiDtoList().items(snippets.stream().map(this::toApiModel).toList())
   ...
   * @return
   */
  private String writeOrder(final SnippetListOrder order) {
    if (null == order) {
      return null;
    } else if (order == SnippetListOrder.CODE_ASC) {
      return "code-asc";
    } else if (order == SnippetListOrder.CODE_DESC) {
      return "code-desc";
    } else {
      return null;
    }
