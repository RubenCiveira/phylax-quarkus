...
public class ThemeAssetsController {
  ...
  public Response serveAsset(@PathParam("themeName") String themeName,
    ...
    Optional<Theme> theme = themes.find(ThemeFilter.builder().name(themeName).build());
    if (theme.isEmpty()) {
      log.debug("Theme '{}' not found", themeName);
      return Response.status(Response.Status.NOT_FOUND).build();
    }

    Optional<ThemeAsset> asset =
        themeAssets.find(ThemeAssetFilter.builder().theme(theme.get()).code(assetCode).build());
    if (asset.isEmpty()) {
