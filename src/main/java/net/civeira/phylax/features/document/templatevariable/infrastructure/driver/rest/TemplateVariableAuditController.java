...
import net.civeira.phylax.generated.openapi.api.TemplateVariableAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class TemplateVariableAuditController implements TemplateVariableAuditApi {
   ...
   */
  @Override
  public Response templateVariableApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName();
    List<AuditEvent> events =
        reader.findByFilters("document_template_variable_audit", "templateVariable",
   ...
   */
  @Override
  public Response templateVariableApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName();
    List<AuditEvent> events =
        reader.findByFilters("document_template_variable_audit", "templateVariable",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("templateVariable").entityType(event.getEntityType())
