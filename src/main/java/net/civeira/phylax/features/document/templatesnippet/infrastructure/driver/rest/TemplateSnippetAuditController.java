...
import net.civeira.phylax.generated.openapi.api.TemplateSnippetAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class TemplateSnippetAuditController implements TemplateSnippetAuditApi {
   ...
   */
  @Override
  public Response templateSnippetApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events =
        reader.findByFilters("document_template_snippet_audit", "templateSnippet",
   ...
   */
  @Override
  public Response templateSnippetApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events =
        reader.findByFilters("document_template_snippet_audit", "templateSnippet",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("templateSnippet").entityType(event.getEntityType())
