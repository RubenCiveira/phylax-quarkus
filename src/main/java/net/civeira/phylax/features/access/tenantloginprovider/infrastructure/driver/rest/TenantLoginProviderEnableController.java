...
public class TenantLoginProviderEnableController {
  ...
  public Response tenantLoginProviderApiBatchEnable(final List<String> uids, final String search,
    ...
    Span span = tracer.spanBuilder("Api to enable on massive entitites of tenant login provider")
  ...
  public Response tenantLoginProviderApiBatchEnableQuery(final String batchId) {
    Span span = tracer.spanBuilder("Api to check masive enable on entity of tenant login provider")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.SERVER).startSpan();
    try {
  ...
  public Response tenantLoginProviderApiEnable(final String uid) {
    Span span = tracer.spanBuilder("Api to enable an entity of tenant login provider")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.SERVER).startSpan();
    try {
