...
public class TenantConfigListController {
  ...
  public Response tenantConfigApiList(final List<String> uids, final String search,
    ...
    Span span = tracer.spanBuilder("Api to list an entity of tenant config")
  ...
  private TenantConfigApiDtoList toListApiModel(List<TenantConfigListProjection> tenantConfigs,
    ...
    Span span = tracer.spanBuilder("Build to api output a list of tenant config")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.INTERNAL)
        .startSpan();
    try {
      ...
      return new TenantConfigApiDtoList()
          .items(tenantConfigs.stream().map(this::toApiModel).toList())
