...
public class RelyingPartyRepository {
   ...
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String RELYING_PARTY_API_KEY = "relying_party.api_key";

  /**
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String RELYING_PARTY_CODE = "relying_party.code";

  /**
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String RELYING_PARTY_ENABLED = "relying_party.enabled";
   ...
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String RELYING_PARTY_UID = "relying_party.uid";

  /**
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String RELYING_PARTY_VERSION = "relying_party.version";
  ...
  public void delete(RelyingParty entity) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq =
          template.createSqlCommand("delete from \"relying_party\" where \"uid\" = :uid");
    ...
    return reference instanceof RelyingParty relyingParty ? relyingParty
        : retrieve(reference.getUidValue(), Optional.empty())
            .orElseThrow(() -> new NotFoundException(
                "Trying to enrich inexistent RelyingParty: " + reference.getUidValue()));
  }
  ...
  public RelyingParty update(RelyingParty entity) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq = template.createSqlCommand(
          "update \"relying_party\" set  \"code\" = :code, \"api_key\" = :apiKey, \"enabled\" = :enabled, \"version\" = \"version\" + 1 where \"uid\" = :uid and \"version\" = :version");
      sq.with(UID, SqlParameterValue.of(entity.getUid().getValue()));
      sq.with(CODE, SqlParameterValue.of(entity.getCode().getValue()));
      sq.with(API_KEY, SqlParameterValue.of(entity.getApiKey().getValue()));
  ...
  private SqlConverter<RelyingParty> converter() {
    return row -> {
      try {
        return Optional.of(RelyingParty.builder().uid(UidVO.from(row.getString(UID)))
            .code(CodeVO.from(row.getString(CODE)))
            .apiKey(ApiKeyVO.from(row.getString(API_KEY_SNAKE)))
  ...
  private <T> SqlSchematicQuery<T> filteredQuery(SqlTemplate template, RelyingPartyFilter filter) {
    SqlSchematicQuery<T> sq = template.createSqlSchematicQuery(RELYING_PARTY_SNAKE);
    sq.selectFields(RELYING_PARTY_UID, RELYING_PARTY_CODE, RELYING_PARTY_API_KEY,
        RELYING_PARTY_ENABLED, RELYING_PARTY_VERSION);
  ...
  private RelyingParty runCreate(RelyingParty entity, Predicate<RelyingParty> verifier) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq = template.createSqlCommand(
          "insert into \"relying_party\" ( \"uid\", \"code\", \"api_key\", \"enabled\", \"version\") values ( :uid, :code, :apiKey, :enabled, :version)");
      sq.with(UID, SqlParameterValue.of(entity.getUid().getValue()));
      sq.with(CODE, SqlParameterValue.of(entity.getCode().getValue()));
      sq.with(API_KEY, SqlParameterValue.of(entity.getApiKey().getValue()));
      sq.with(ENABLED, entity.getEnabled().getValue().map(SqlParameterValue::of)
