...
public class TenantLoginProviderUpdateInput {
  ...
  TenantLoginProviderChangeSet toChangeSet() {
    return new TenantLoginProviderChangeSet().uid(uid).tenant(tenant).name(name).source(source)
        ...
        .certificate(certificate).metadata(metadata).samlIdpMetadataUrl(samlIdpMetadataUrl)
        .samlIdpEntityId(samlIdpEntityId).samlIdpSsoUrl(samlIdpSsoUrl)
        .samlIdpIdpCert(samlIdpIdpCert).usersEnabledByDefault(usersEnabledByDefault)
        .version(version);
  }
}
