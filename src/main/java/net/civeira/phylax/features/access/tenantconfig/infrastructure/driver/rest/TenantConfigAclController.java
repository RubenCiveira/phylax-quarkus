...
public class TenantConfigAclController implements TenantConfigAclApi {
  ...
  public Response tenantConfigApiContextualAcl(final String uid) {
    Span span = tracer.spanBuilder("Api to check acl an entity of tenant config")
  ...
  public Response tenantConfigApiGenericAcl() {
    Span span = tracer.spanBuilder("Api to check acl of a recorf on an entity of tenant config")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.SERVER).startSpan();
    try {
      Interaction interaction = currentRequest.interaction();
