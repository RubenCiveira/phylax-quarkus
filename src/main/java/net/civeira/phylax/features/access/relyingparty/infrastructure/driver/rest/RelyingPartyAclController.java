...
public class RelyingPartyAclController implements RelyingPartyAclApi {
  ...
  public Response relyingPartyApiContextualAcl(final String uid) {
    Span span = tracer.spanBuilder("Api to check acl an entity of relying party")
  ...
  public Response relyingPartyApiGenericAcl() {
    Span span = tracer.spanBuilder("Api to check acl of a recorf on an entity of relying party")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.SERVER).startSpan();
    try {
      Interaction interaction = currentRequest.interaction();
