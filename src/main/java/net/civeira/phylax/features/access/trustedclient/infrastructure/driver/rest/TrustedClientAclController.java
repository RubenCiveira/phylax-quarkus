...
public class TrustedClientAclController implements TrustedClientAclApi {
  ...
  public Response trustedClientApiContextualAcl(final String uid) {
    Span span = tracer.spanBuilder("Api to check acl an entity of trusted client")
  ...
  public Response trustedClientApiGenericAcl() {
    Span span = tracer.spanBuilder("Api to check acl of a recorf on an entity of trusted client")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.SERVER).startSpan();
    try {
      Interaction interaction = currentRequest.interaction();
