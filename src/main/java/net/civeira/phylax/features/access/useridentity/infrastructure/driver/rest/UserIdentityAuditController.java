...
import net.civeira.phylax.generated.openapi.api.UserIdentityAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class UserIdentityAuditController implements UserIdentityAuditApi {
   ...
   */
  @Override
  public Response userIdentityApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("access_user_identity_audit", "userIdentity",
   ...
   */
  @Override
  public Response userIdentityApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("access_user_identity_audit", "userIdentity",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("userIdentity").entityType(event.getEntityType()).entityId(event.getEntityId())
