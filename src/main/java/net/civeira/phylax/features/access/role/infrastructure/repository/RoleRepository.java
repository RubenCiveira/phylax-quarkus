...
public class RoleRepository {
   ...
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String ACCESS_ROLE_TENANT = "access_role.tenant";
  ...
  public List<SecurityDomain> resolveDomains(final List<Domains> childs) {
    return securityDomainRepository.list(
        "select \"access_security_domain\".* from \"access_security_domain\" join \"access_role_domain\" on \"access_security_domain\".\"uid\" = \"access_role_domain\".\"security_domain\" "
            + " where \"access_role_domain\".\"uid\" in ( :availables )",
        Map.of("availables",
            SqlListParameterValue.strings(childs.stream().map(Domains::getUid).toList())));
  }
  ...
  public Role update(Role entity) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq = template.createSqlCommand(
          "update \"access_role\" set  \"name\" = :name, \"tenant\" = :tenant, \"version\" = \"version\" + 1 where \"uid\" = :uid and \"version\" = :version");
      sq.with(UID, SqlParameterValue.of(entity.getUid()));
      ...
      Role result = entity.withNextVersion();
      saveDomains(result, result.getDomains());
      return result;
    }
  }
    ...
    return row -> {
      try {
        ...
        return Optional.of(Role.builder().uidValue(UidVO.from(row.getString(UID)))
            ...
            .domainsValue(DomainsVO.from(domainsFromChilds))
  ...
  private void deleteDomains(Role entity, SqlTemplate template) {
    SqlCommand sq =
        template.createSqlCommand("delete from \"access_role_domain\" where \"role\" = :role");
    sq.with(ROLE, SqlParameterValue.of(entity.getUid()));
    sq.execute();
  }
  ...
  private <T> SqlSchematicQuery<T> filteredQuery(SqlTemplate template, RoleFilter filter) {
    ...
    sq.selectFields(ACCESS_ROLE_UID, ACCESS_ROLE_NAME, ACCESS_ROLE_TENANT, ACCESS_ROLE_VERSION);
    withChilds(sq);
    ...
    filter.getTenantTenantAccesible().ifPresent(tenantTenantAccesible -> sq
        .where(ACCESS_ROLE_TENANT, SqlOperator.EQ, SqlParameterValue.of(tenantTenantAccesible)));
    return sq;
  }
  ...
  private Role runCreate(Role entity, Predicate<Role> verifier) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq = template.createSqlCommand(
          "insert into \"access_role\" ( \"uid\", \"name\", \"tenant\", \"version\") values ( :uid, :name, :tenant, :version)");
      sq.with(UID, SqlParameterValue.of(entity.getUid()));
      sq.with(NAME, SqlParameterValue.of(entity.getName()));
      sq.with(TENANT, entity.getTenantUid().map(SqlParameterValue::of)
          .orElseGet(SqlParameterValue::ofNullString));
      ...
      result = saveDomains(result, result.getDomains());
      return result;
    }
  }
  ...
  private Role saveDomains(Role entity, List<Domains> childs) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlQuery<Object[]> sq = template.createSqlQuery(
          "select \"uid\", \"role\" from \"access_role_domain\" where (\"role\" = :role)"
              + " or ( \"uid\" in (:tomap) and \"role\" != :role )");
      sq.with(ROLE, SqlParameterValue.of(entity.getUid()));
      sq.with(TOMAP, SqlListParameterValue.strings(childs.stream().map(Domains::getUid).toList()));
      List<Object[]> list =
      ...
      if (!codes.isEmpty()) {
        SqlCommand deleted = template
            .createSqlCommand("delete from\"access_role_domain\" where \"uid\" in (:codes)");
      ...
      childs.forEach(child -> {
        SqlCommand command = template.createSqlCommand(codes.contains(child.getUid())
            ? "update \"access_role_domain\" set \"verify\" = :verify, \"medal\" = :medal, \"version\" = \"version\" + 1 where \"uid\" = :uid and \"version\" = :version"
            : "insert into \"access_role_domain\" ( \"uid\", \"role\", \"security_domain\", \"version\") values ( :uid, :role, :securityDomain, :version)");
        command.with(UID, SqlParameterValue.of(child.getUid()));
        command.with(ROLE, SqlParameterValue.of(entity.getUid()));
        command.with(SECURITY_DOMAIN, SqlParameterValue.of(child.getSecurityDomainUid()));
  ...
  private void withChilds(AbstractSqlParametrized<?> sq) {
    sq.child(100, "domains",
        "select \"role\",\"uid\",\"security_domain\",\"version\" from \"access_role_domain\"  where \"role\" in (:role)",
        "uid", "role", row -> {
          try {
