...
public class UserImportationService {
  ...
  public UserChangeSet toChangeSet(final Map<String, String> row, final ConstraintFailList fails,
    ...
    change = readEmailVerified(change, row, fails, resolver);
   ...
   * @return
   */
  private UserChangeSet readEmailVerified(final UserChangeSet change, final Map<String, String> row,
      final ConstraintFailList fails, final UserImportationRelatedResolver resolver) {
    String emailVerified = row.get("emailVerified");
    if (emailVerified != null) {
      return change.emailVerified(EmailVerifiedVO.tryFromString(emailVerified, fails));
    } else {
      return change.unsetEmailVerified();
    }
  }
