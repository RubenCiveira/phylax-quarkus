...
import net.civeira.phylax.features.access.role.application.usecase.delete.RoleDeleteAllInBatchCommand;
...
public class RoleDeleteController {
     ...
     * @return
     */
    @ApiObserved("Api to delete on massive entity of role")
    public Response roleApiBatchDelete(final List<String> uids, final String search, final String name, final String relyingParty, final List<String> relyingPartys) {
        ...
        filterBuilder = filterBuilder.uids(uids.stream().flatMap(part -> Stream.of(part.split(","))).toList());
    ...
    public Response roleApiBatchDeleteQuery(final String batchId) {
        BatchProgress task = delete.checkProgress(RoleCheckBatchDeleteStatus.builder().taskId(batchId).build(currentRequest.interaction()));
        ...
        response.setSteps(task.getSteps().stream().map(step -> {
            ...
            if (null != stepErrors) {
                bsp.getErrors().addAll(stepErrors.entrySet().stream().map(entry -> new BatchResultErrorItem().item(entry.getKey()).fails(entry.getValue().getFails().stream().map(fail -> new MultipleErrorCodes().code(fail.getCode()).violation(fail.getViolation()).wrongValues(fail.getWrongValues().stream().map(wv -> new ConstraintFail().error(wv.getErrorMessage()).field(wv.getField()).wrong(String.valueOf(wv.getWrongValue()))).toList())).toList())).toList());
            }
            Map<String, ErrorInfo> stepWarns = step.getWarns();
            if (null != stepWarns) {
                bsp.getWarns().addAll(stepErrors.entrySet().stream().map(entry -> new BatchResultErrorItem().item(entry.getKey()).fails(entry.getValue().getFails().stream().map(fail -> new MultipleErrorCodes().code(fail.getCode()).violation(fail.getViolation()).wrongValues(fail.getWrongValues().stream().map(wv -> new ConstraintFail().error(wv.getErrorMessage()).field(wv.getField()).wrong(String.valueOf(wv.getWrongValue()))).toList())).toList())).toList());
            }
            return bsp;
        }).toList());
     ...
     */
    @Transactional
    @ApiObserved("Api to delete an entity of role")
