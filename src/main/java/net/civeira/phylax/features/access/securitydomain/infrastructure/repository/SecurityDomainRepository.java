...
public class SecurityDomainRepository {
   ...
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String SECURITY_DOMAIN_ENABLED = "security_domain.enabled";

  /**
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String SECURITY_DOMAIN_LEVEL = "security_domain.level";

  /**
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String SECURITY_DOMAIN_MANAGE_ALL = "security_domain.manage_all";

  /**
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String SECURITY_DOMAIN_NAME = "security_domain.name";

  /**
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String SECURITY_DOMAIN_READ_ALL = "security_domain.read_all";
   ...
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String SECURITY_DOMAIN_UID = "security_domain.uid";

  /**
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String SECURITY_DOMAIN_VERSION = "security_domain.version";

  /**
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String SECURITY_DOMAIN_WRITE_ALL = "security_domain.write_all";
  ...
  public void delete(SecurityDomain entity) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq =
          template.createSqlCommand("delete from \"security_domain\" where \"uid\" = :uid");
   ...
   * @return
   */
  public SecurityDomain enrich(SecurityDomainRef reference) {
    return reference instanceof SecurityDomain securityDomain ? securityDomain
        : retrieve(reference.getUidValue(), Optional.empty())
            .orElseThrow(() -> new NotFoundException(
                "Trying to enrich inexistent SecurityDomain: " + reference.getUidValue()));
  }
  ...
  public SecurityDomain update(SecurityDomain entity) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq = template.createSqlCommand(
          "update \"security_domain\" set  \"name\" = :name, \"level\" = :level, \"read_all\" = :readAll, \"write_all\" = :writeAll, \"manage_all\" = :manageAll, \"enabled\" = :enabled, \"version\" = \"version\" + 1 where \"uid\" = :uid and \"version\" = :version");
      sq.with(UID, SqlParameterValue.of(entity.getUid().getValue()));
      sq.with(NAME, SqlParameterValue.of(entity.getName().getValue()));
      sq.with(LEVEL, SqlParameterValue.of(entity.getLevel().getValue()));
      sq.with(READ_ALL, entity.getReadAll().getValue().map(SqlParameterValue::of)
          .orElseGet(SqlParameterValue::ofNullBoolean));
      sq.with(WRITE_ALL, entity.getWriteAll().getValue().map(SqlParameterValue::of)
          .orElseGet(SqlParameterValue::ofNullBoolean));
      sq.with(MANAGE_ALL, entity.getManageAll().getValue().map(SqlParameterValue::of)
          .orElseGet(SqlParameterValue::ofNullBoolean));
  ...
  private SqlConverter<SecurityDomain> converter() {
    return row -> {
      try {
        return Optional.of(SecurityDomain.builder().uid(UidVO.from(row.getString(UID)))
            .name(NameVO.from(row.getString(NAME))).level(LevelVO.from(row.getInt(LEVEL)))
            .readAll(ReadAllVO.from(row.getBoolean(READ_ALL_SNAKE)))
            .writeAll(WriteAllVO.from(row.getBoolean(WRITE_ALL_SNAKE)))
            .manageAll(ManageAllVO.from(row.getBoolean(MANAGE_ALL_SNAKE)))
  ...
  private <T> SqlSchematicQuery<T> filteredQuery(SqlTemplate template,
    ...
    SqlSchematicQuery<T> sq = template.createSqlSchematicQuery(SECURITY_DOMAIN_SNAKE);
    sq.selectFields(SECURITY_DOMAIN_UID, SECURITY_DOMAIN_NAME, SECURITY_DOMAIN_LEVEL,
        SECURITY_DOMAIN_READ_ALL, SECURITY_DOMAIN_WRITE_ALL, SECURITY_DOMAIN_MANAGE_ALL,
        SECURITY_DOMAIN_ENABLED, SECURITY_DOMAIN_VERSION);
  ...
  private SecurityDomain runCreate(SecurityDomain entity, Predicate<SecurityDomain> verifier) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq = template.createSqlCommand(
          "insert into \"security_domain\" ( \"uid\", \"name\", \"level\", \"read_all\", \"write_all\", \"manage_all\", \"enabled\", \"version\") values ( :uid, :name, :level, :readAll, :writeAll, :manageAll, :enabled, :version)");
      sq.with(UID, SqlParameterValue.of(entity.getUid().getValue()));
      sq.with(NAME, SqlParameterValue.of(entity.getName().getValue()));
      sq.with(LEVEL, SqlParameterValue.of(entity.getLevel().getValue()));
      sq.with(READ_ALL, entity.getReadAll().getValue().map(SqlParameterValue::of)
