...
import net.civeira.phylax.generated.openapi.api.ConsentPurposeAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class ConsentPurposeAuditController implements ConsentPurposeAuditApi {
   ...
   */
  @Override
  public Response consentPurposeApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("access_consent_purpose_audit", "consentPurpose",
   ...
   */
  @Override
  public Response consentPurposeApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("access_consent_purpose_audit", "consentPurpose",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("consentPurpose").entityType(event.getEntityType())
