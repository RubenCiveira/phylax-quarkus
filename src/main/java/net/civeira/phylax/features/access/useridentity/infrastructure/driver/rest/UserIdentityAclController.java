...
public class UserIdentityAclController implements UserIdentityAclApi {
  ...
  public Response userIdentityApiContextualAcl(final String uid) {
    Span span = tracer.spanBuilder("Api to check acl an entity of user identity")
  ...
  public Response userIdentityApiGenericAcl() {
    Span span = tracer.spanBuilder("Api to check acl of a recorf on an entity of user identity")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.SERVER).startSpan();
    try {
      Interaction interaction = currentRequest.interaction();
