...
public class RelyingPartyListController {
  ...
  public Response relyingPartyApiList(final List<String> uids, final String search,
    ...
    Span span = tracer.spanBuilder("Api to list an entity of relying party")
  ...
  private RelyingPartyApiDtoList toListApiModel(List<RelyingPartyListProjection> relyingPartys,
    ...
    Span span = tracer.spanBuilder("Build to api output a list of relying party")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.INTERNAL)
        .startSpan();
    try {
      ...
      filter.getApiKey().ifPresent(filterApiKey -> {
        String apiKeyValue =
            URLEncoder.encode(String.valueOf(filterApiKey), StandardCharsets.UTF_8);
      ...
      return new RelyingPartyApiDtoList()
          .items(relyingPartys.stream().map(this::toApiModel).toList())
