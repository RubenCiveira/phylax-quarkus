...
public class UserIdentityListController {
  ...
  public Response userIdentityApiList(final List<String> uids, final String search,
    ...
    Span span = tracer.spanBuilder("Api to list an entity of user identity")
  ...
  private UserIdentityApiDtoList toListApiModel(List<UserIdentityListProjection> userIdentitys,
    ...
    Span span = tracer.spanBuilder("Build to api output a list of user identity")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.INTERNAL)
        .startSpan();
    try {
      ...
      return new UserIdentityApiDtoList()
          .items(userIdentitys.stream().map(this::toApiModel).toList())
