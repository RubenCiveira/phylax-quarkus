...
import net.civeira.phylax.features.access.user.application.usecase.accept.UserAcceptProjection;
import net.civeira.phylax.features.access.user.application.usecase.accept.UserAcceptStatus;
...
public class UserAcceptController {
  ...
  public Response userApiAccept(final String uid) {
    Span span = tracer.spanBuilder("Api to accept an entity of user")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.SERVER).startSpan();
    try {
      span.setAttribute("ref", uid);
      UserAcceptProjection updated =
          accept.accept(currentRequest.interaction(), UserReference.of(uid));
  ...
  public Response userApiBatchAccept(final List<String> uids, final String search,
    ...
    Span span = tracer.spanBuilder("Api to accept on massive entitites of user")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.SERVER).startSpan();
    try {
  ...
  public Response userApiBatchAcceptQuery(final String batchId) {
    Span span = tracer.spanBuilder("Api to check masive accept on entity of user")
        .setParent(Context.current().with(Span.current())).setSpanKind(SpanKind.SERVER).startSpan();
    try {
   ...
   * @return
   */
  private UserApiDto toApiModel(UserAcceptProjection dto) {
    Span span =
