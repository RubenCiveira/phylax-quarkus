...
public class ConsentPurposeAuditAdapter implements ConsentPurposeAuditGateway {
  ...
  public void created(final String usecase, final ConsentPurpose consentPurpose,
    ...
    writer.record("_audit_access_consent_purpose",
        AuditEvent.builder().operation("create").usecase(usecase).traceId(currentTraceId())
            .spanId(currentSpanId()).entityType("consentPurpose").entityId(consentPurpose.getUid())
            .newValue(consentPurpose.toMap()).performedBy(context.getActor().getName())
  ...
  public void deleted(final String usecase, final ConsentPurpose consentPurpose,
    ...
    writer.record("_audit_access_consent_purpose",
        AuditEvent.builder().operation("delete").usecase(usecase).traceId(currentTraceId())
            .spanId(currentSpanId()).entityType("consentPurpose").entityId(consentPurpose.getUid())
            .oldValue(consentPurpose.toMap()).performedBy(context.getActor().getName())
            .tenant(context.getActor().getTenant()).timestamp(context.getSource().getStartTime())
            .sourceRequest(context.getSource().getRequestId())
            .remoteAddress(context.getSource().getIp())
            .remoteApplication(context.getSource().getApplication())
