...
import net.civeira.phylax.generated.openapi.api.PlatformIdentityAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class PlatformIdentityAuditController implements PlatformIdentityAuditApi {
   ...
   */
  @Override
  public Response platformIdentityApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events =
        reader.findByFilters("access_platform_identity_audit", "platformIdentity",
   ...
   */
  @Override
  public Response platformIdentityApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events =
        reader.findByFilters("access_platform_identity_audit", "platformIdentity",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("platformIdentity").entityType(event.getEntityType())
