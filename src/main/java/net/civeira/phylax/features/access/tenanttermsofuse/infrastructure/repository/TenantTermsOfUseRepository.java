...
public class TenantTermsOfUseRepository {
   ...
   * @autogenerated RepositoryJdbcGenerator
   */
  private static final String ACCESS_TENANT_TERMS_OF_USE_RELYING_PARTY =
      "access_tenant_terms_of_use.relying_party";
  ...
  public TenantTermsOfUse update(TenantTermsOfUse entity) {
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq = template.createSqlCommand(
          "update \"access_tenant_terms_of_use\" set  \"tenant\" = :tenant, \"relying_party\" = :relyingParty, \"text\" = :text, \"enabled\" = :enabled, \"attached\" = :attached, \"activation_date\" = :activationDate, \"version\" = \"version\" + 1 where \"uid\" = :uid and \"version\" = :version");
      sq.with(UID, SqlParameterValue.of(entity.getUid()));
  ...
  private <T> SqlSchematicQuery<T> filteredQuery(SqlTemplate template,
    ...
    sq.selectFields(ACCESS_TENANT_TERMS_OF_USE_UID, ACCESS_TENANT_TERMS_OF_USE_TENANT,
        ACCESS_TENANT_TERMS_OF_USE_RELYING_PARTY, ACCESS_TENANT_TERMS_OF_USE_TEXT,
        ACCESS_TENANT_TERMS_OF_USE_ENABLED, ACCESS_TENANT_TERMS_OF_USE_ATTACHED,
  ...
  private TenantTermsOfUse runCreate(TenantTermsOfUse entity,
    ...
    try (SqlTemplate template = new SqlTemplate(datasource, tracer)) {
      SqlCommand sq = template.createSqlCommand(
          "insert into \"access_tenant_terms_of_use\" ( \"uid\", \"tenant\", \"relying_party\", \"text\", \"enabled\", \"attached\", \"activation_date\", \"version\") values ( :uid, :tenant, :relyingParty, :text, :enabled, :attached, :activationDate, :version)");
      sq.with(UID, SqlParameterValue.of(entity.getUid()));
      sq.with(TENANT, SqlParameterValue.of(entity.getTenantUid()));
      sq.with(RELYING_PARTY, entity.getRelyingPartyUid().map(SqlParameterValue::of)
          .orElseGet(SqlParameterValue::ofNullString));
