package net.civeira.phylax.features.access.useracceptedtermnsofuse.query;

import java.util.List;
import java.util.Optional;

import lombok.Builder;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.With;
import net.civeira.phylax.features.access.tenanttermsofuse.TenantTermsOfUseRef;
import net.civeira.phylax.features.access.user.UserRef;

@Data
@Builder(toBuilder = true)
@With
@EqualsAndHashCode(onlyExplicitlyIncluded = true)
public class UserAcceptedTermnsOfUseFilter {

  /**
   * @autogenerated FilterGenerator
   */
  private TenantTermsOfUseRef conditions;

  /**
   * @autogenerated FilterGenerator
   */
  private String conditionsTenantTenantAccesible;

  /**
   * @autogenerated FilterGenerator
   */
  private List<String> conditionss;

  /**
   * The filter to select the results to retrieve
   *
   * @autogenerated FilterGenerator
   */
  private String search;

  /**
   * The filter to select the results to retrieve
   *
   * @autogenerated FilterGenerator
   */
  private String uid;

  /**
   * The filter to select the results to retrieve
   *
   * @autogenerated FilterGenerator
   */
  private List<String> uids;

  /**
   * @autogenerated FilterGenerator
   */
  private UserRef user;

  /**
   * @autogenerated FilterGenerator
   */
  private List<String> users;

  /**
   * @autogenerated FilterGenerator
   * @return
   */
  public Optional<TenantTermsOfUseRef> getConditions() {
    return Optional.ofNullable(conditions);
  }

  /**
   * @autogenerated FilterGenerator
   * @return
   */
  public Optional<String> getConditionsTenantTenantAccesible() {
    return Optional.ofNullable(conditionsTenantTenantAccesible);
  }

  /**
   * @autogenerated FilterGenerator
   * @return
   */
  public List<String> getConditionss() {
    return null == conditionss ? List.of() : List.copyOf(conditionss);
  }

  /**
   * @autogenerated FilterGenerator
   * @return
   */
  public Optional<String> getSearch() {
    return Optional.ofNullable(search);
  }

  /**
   * @autogenerated FilterGenerator
   * @return
   */
  public Optional<String> getUid() {
    return Optional.ofNullable(uid);
  }

  /**
   * @autogenerated FilterGenerator
   * @return
   */
  public List<String> getUids() {
    return null == uids ? List.of() : List.copyOf(uids);
  }

  /**
   * @autogenerated FilterGenerator
   * @return
   */
  public Optional<UserRef> getUser() {
    return Optional.ofNullable(user);
  }

  /**
   * @autogenerated FilterGenerator
   * @return
   */
  public List<String> getUsers() {
    return null == users ? List.of() : List.copyOf(users);
  }
}
