...
import net.civeira.phylax.features.access.platformidentity.application.usecase.list.PlatformIdentityListCursor;
import net.civeira.phylax.features.access.platformidentity.application.usecase.list.PlatformIdentityListFilter;
import net.civeira.phylax.features.access.platformidentity.application.usecase.list.PlatformIdentityListProjection;
...
import net.civeira.phylax.generated.openapi.model.PlatformIdentityApiDtoList;
...
@ApplicationScoped
public class PlatformIdentityListController {
   ...
   * @return
   */
  @ApiObserved("Api to list an entity of platform identity")
  public Response platformIdentityApiList(final List<String> uids, final String search,
    ...
    PlatformIdentityListFilter.PlatformIdentityListFilterBuilder filterBuilder =
        PlatformIdentityListFilter.builder();
    PlatformIdentityListCursor.PlatformIdentityListCursorBuilder cursorBuilder =
        PlatformIdentityListCursor.builder();
    ...
    PlatformIdentityListFilter filter = filterBuilder.build();
    PlatformIdentityListCursor cursor = cursorBuilder.build();
    List<PlatformIdentityListProjection> listed =
    ...
    return currentRequest.cacheableResponse(listed, toListApiModel(listed, filter, cursor),
        "platform-identity-" + ("" + filter + cursor).hashCode());
  }
   ...
   * @return
   */
  private PlatformIdentityApiDto toApiModel(PlatformIdentityListProjection dto) {
   ...
   * @return
   */
  private PlatformIdentityApiDtoList toListApiModel(
      List<PlatformIdentityListProjection> platformIdentitys, PlatformIdentityListFilter filter,
      PlatformIdentityListCursor cursor) {
    Optional<PlatformIdentityListProjection> last = platformIdentitys.isEmpty() ? Optional.empty()
        : Optional.of(platformIdentitys.get(platformIdentitys.size() - 1));
    ...
    return new PlatformIdentityApiDtoList()
        .items(platformIdentitys.stream().map(this::toApiModel).toList())
