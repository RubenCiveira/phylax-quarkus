...
public class ClientIdentityImportationService {
   ...
   * @autogenerated VisibilityFilterGenerator
   */
  private final ClientIdentityWriteRepositoryGateway repository;
   ...
   * @return
   */
  public ClientIdentityImportationResult load(final Map<String, String> row) {
    return load(row, ClientIdentityImportationRelatedResolver.builder().build());
  }
   ...
   */
  public ClientIdentityImportationResult load(final Map<String, String> row,
      final ClientIdentityImportationRelatedResolver resolver) {
    ...
    ClientIdentity entity = null;
    if (!fails.isEmpty()) {
      Optional<ClientIdentity> existing =
          repository.findForUpdate(ClientIdentityFilter.builder().uid(uid).build());
      ClientIdentityChangeSet change = toChangeSet(row, fails, resolver);
      if (existing.isPresent()) {
        entity = existing.get().update(change);
        exists = true;
      } else {
        entity = ClientIdentity.create(change.newUid());
      }
    }
    return fails.isEmpty() ? new ClientIdentityImportationResult(entity, exists)
        : new ClientIdentityImportationResult(fails);
  }
   ...
   * @return
   */
  public ClientIdentityChangeSet toChangeSet(final Map<String, String> row,
      final ConstraintFailList fails, final ClientIdentityImportationRelatedResolver resolver) {
    ClientIdentityChangeSet change = new ClientIdentityChangeSet();
    ...
    change = readRoles(change, row, fails, resolver);
    return change;
  }
   ...
   * @return
   */
  private ClientIdentityChangeSet readRelyingParty(final ClientIdentityChangeSet change,
   ...
   * @return
   */
  private ClientIdentityChangeSet readRoles(final ClientIdentityChangeSet change,
      final Map<String, String> row, final ConstraintFailList fails,
      final ClientIdentityImportationRelatedResolver resolver) {
    String roles = row.get("roles");
    if (roles != null) {
      return change.roles(roles);
    } else {
      return change.unsetRoles();
    }
  }
   ...
   * @return
   */
  private ClientIdentityChangeSet readTrustedClient(final ClientIdentityChangeSet change,
      final Map<String, String> row, final ConstraintFailList fails,
      final ClientIdentityImportationRelatedResolver resolver) {
   ...
   * @return
   */
  private ClientIdentityChangeSet readUser(final ClientIdentityChangeSet change,
      final Map<String, String> row, final ConstraintFailList fails,
      final ClientIdentityImportationRelatedResolver resolver) {
