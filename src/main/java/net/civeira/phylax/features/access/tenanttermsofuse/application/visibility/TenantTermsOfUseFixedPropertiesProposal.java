package net.civeira.phylax.features.access.tenanttermsofuse.application.visibility;

import java.util.Optional;
import java.util.Set;

import lombok.Getter;
import net.civeira.phylax.common.security.Actor;
import net.civeira.phylax.common.security.InvocationSource;
import net.civeira.phylax.common.security.OperationContext;
import net.civeira.phylax.common.security.PropertiesProposal;
import net.civeira.phylax.features.access.tenanttermsofuse.domain.TenantTermsOfUseRef;

@Getter
public class TenantTermsOfUseFixedPropertiesProposal extends PropertiesProposal {

  /**
   * @autogenerated FixedPropertiesProposalGenerator
   */
  private final TenantTermsOfUseRef reference;

  /**
   * @autogenerated FixedPropertiesProposalGenerator
   * @param context
   * @param reference
   * @param fields
   */
  public TenantTermsOfUseFixedPropertiesProposal(final OperationContext context,
      final TenantTermsOfUseRef reference, final Set<String> fields) {
    super(context, fields);
    this.reference = reference;
  }

  /**
   * @autogenerated FixedPropertiesProposalGenerator
   * @param context
   * @param reference
   */
  public TenantTermsOfUseFixedPropertiesProposal(final OperationContext context,
      final TenantTermsOfUseRef reference) {
    super(context);
    this.reference = reference;
  }

  /**
   * @autogenerated FixedPropertiesProposalGenerator
   * @param context
   * @param fields
   */
  public TenantTermsOfUseFixedPropertiesProposal(final OperationContext context,
      final Set<String> fields) {
    super(context, fields);
    this.reference = null;
  }

  /**
   * @autogenerated FixedPropertiesProposalGenerator
   * @return
   */
  public Actor getActor() {
    return this.context.getActor();
  }

  /**
   * @autogenerated FixedPropertiesProposalGenerator
   * @return
   */
  public Optional<TenantTermsOfUseRef> getReference() {
    return Optional.ofNullable(this.reference);
  }

  /**
   * @autogenerated FixedPropertiesProposalGenerator
   * @return
   */
  public InvocationSource getSource() {
    return this.context.getSource();
  }

  /**
   * @autogenerated FixedPropertiesProposalGenerator
   * @return
   */
  public String resourceName() {
    return "tenant-terms-of-use";
  }

  /**
   * @autogenerated FixedPropertiesProposalGenerator
   * @return
   */
  public String viewName() {
    return "update";
  }
}
