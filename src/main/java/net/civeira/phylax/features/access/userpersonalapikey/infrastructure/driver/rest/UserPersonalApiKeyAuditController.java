...
import net.civeira.phylax.generated.openapi.api.UserPersonalApiKeyAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class UserPersonalApiKeyAuditController implements UserPersonalApiKeyAuditApi {
   ...
   */
  @Override
  public Response userPersonalApiKeyApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events =
        reader.findByFilters("access_user_personal_api_key_audit", "userPersonalApiKey",
   ...
   */
  @Override
  public Response userPersonalApiKeyApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events =
        reader.findByFilters("access_user_personal_api_key_audit", "userPersonalApiKey",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("userPersonalApiKey").entityType(event.getEntityType())
