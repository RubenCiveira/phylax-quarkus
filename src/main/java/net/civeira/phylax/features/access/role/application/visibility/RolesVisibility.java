...
public class RolesVisibility {
    ...
    public boolean checkVisibility(Interaction prev, List<String> uids) {
        return uids.size() == listVisibles(prev, RoleVisibilityFilter.builder().uids(uids).build(), RoleCursor.builder().build()).size();
    }
    ...
    public RoleChangeSet copyWithFixed(Interaction prev, RoleChangeSet source, Role original) {
        ...
        RoleEntityEnrichment proposal = RoleEntityEnrichment.builder().dto(withReferences).interaction(prev).original(original).build();
    ...
    public RoleChangeSet copyWithFixed(Interaction prev, RoleChangeSet source) {
        ...
        RoleEntityEnrichment proposal = RoleEntityEnrichment.builder().dto(withReferences).interaction(prev).build();
        presetEmitter.fire(proposal);
        return proposal.getDto();
    }
    ...
    public Set<String> fieldsToFix(Interaction prev) {
        ...
        RoleFixedFieldsCheck proposal = RoleFixedFieldsCheck.builder().fields(fields).query(prev).build();
    ...
    public Set<String> fieldsToFix(Interaction prev, RoleRef role) {
        ...
        RoleFixedFieldsCheck proposal = RoleFixedFieldsCheck.builder().fields(fields).role(role).query(prev).build();
        fixedFieldsEmitter.fire(proposal);
        return proposal.getFields();
    }
    ...
    public Set<String> fieldsToHide(Interaction prev) {
        ...
        RoleHiddenFieldsCheck proposal = RoleHiddenFieldsCheck.builder().fields(fields).query(prev).build();
    ...
    public Set<String> fieldsToHide(Interaction prev, RoleRef role) {
        Set<String> fields = new HashSet<>(fieldsToHide(prev));
        RoleHiddenFieldsCheck proposal = RoleHiddenFieldsCheck.builder().fields(fields).role(role).query(prev).build();
        hiddenFieldsEmitter.fire(proposal);
        return proposal.getFields();
    }
     ...
     * @return The input entity with the copy values without hidden
     */
    public RoleCached listCachedVisibles(Interaction prev, RoleVisibilityFilter filter, RoleCursor cursor) {
        RoleFilter visibleFilter = applyPreVisibilityFilter(prev, filter);
        return cache.retrieve(visibleFilter, cursor).orElseGet(() -> cache.store(visibleFilter, cursor, listVisibles(prev, filter, cursor)));
    }
     ...
     * @return The input entity with the copy values without hidden
     */
    public List<Role> listVisiblesForUpdate(Interaction prev, RoleVisibilityFilter filter, RoleCursor cursor) {
    ...
    public RoleCached retrieveCachedVisible(Interaction prev, String uid) {
        ...
        RoleFilter visibleFilter = applyPreVisibilityFilter(prev, RoleVisibilityFilter.builder().uid(uid).build());
    ...
    public Optional<Role> retrieveVisible(Interaction prev, String uid) {
        RoleFilter filter = applyPreVisibilityFilter(prev, RoleVisibilityFilter.builder().uid(uid).build());
    ...
    private RoleFilter applyPreVisibilityFilter(Interaction prev, RoleVisibilityFilter filter) {
        RoleVisibilityCheck proposal = RoleVisibilityCheck.builder().filter(filter).interaction(prev).build();
        filterProposalEmitter.fire(proposal);
        RoleVisibilityFilter modified = proposal.getFilter();
        return RoleFilter.builder().uid(modified.getUid().orElse(null)).uids(modified.getUids().stream().toList()).search(modified.getSearch().orElse(null)).name(modified.getName().orElse(null)).relyingParty(modified.getRelyingParty().orElse(null)).relyingPartys(modified.getRelyingPartys()).build();
    }
    ...
    private boolean evaluatePostVisibility(Interaction prev, Role roleRef) {
        RoleAccesibleCheck proposal = RoleAccesibleCheck.builder().visible(true).entity(roleRef).interaction(prev).build();
    ...
    private Optional<Role> queryItem(Interaction prev, String uid, RoleFilter filter) {
        return roleReadRepositoryGateway.retrieve(uid, Optional.of(filter)).filter(values -> evaluatePostVisibility(prev, values));
    }
    ...
    private Optional<Role> queryItemForUpdate(Interaction prev, String uid, RoleFilter filter) {
        return roleWriteRepositoryGateway.retrieveForUpdate(uid, Optional.of(filter)).filter(values -> evaluatePostVisibility(prev, values));
    }
    ...
    private List<Role> queryItems(Interaction prev, RoleFilter filter, RoleCursor cursor) {
        ...
        Iterator<Role> slide = roleReadRepositoryGateway.slide(filter, cursor).slide(values -> evaluatePostVisibility(prev, values));
        while (slide.hasNext()) {
            list.add(slide.next());
        }
        return list;
    ...
    private List<Role> queryItemsForUpdate(Interaction prev, RoleFilter filter, RoleCursor cursor) {
        List<Role> list = new ArrayList<>();
        Iterator<Role> slide = roleWriteRepositoryGateway.slideForUpdate(filter, cursor).slide(values -> evaluatePostVisibility(prev, values));
        while (slide.hasNext()) {
            list.add(slide.next());
        }
        return list;
