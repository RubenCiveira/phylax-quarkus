...
public class TenantsVisibility {
   ...
   * @return The input entity with the copy values without hidden
   */
  public TenantChangeSet copyWithFixed(Interaction prev, Tenant original, TenantChangeSet source) {
    fieldsToFix(prev, original).forEach(source::unset);
    TenantEntityEnrichment proposal =
        TenantEntityEnrichment.builder().dto(source).interaction(prev).original(original).build();
  ...
  public TenantChangeSet copyWithFixed(Interaction prev, TenantChangeSet source) {
    ...
    TenantEntityEnrichment proposal =
        TenantEntityEnrichment.builder().dto(source).interaction(prev).build();
    presetEmitter.fire(proposal);
    return proposal.getDto();
  }
