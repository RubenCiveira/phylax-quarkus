...
import net.civeira.phylax.generated.openapi.api.ClientIdentityAuditApi;
import net.civeira.phylax.generated.openapi.model.AuditEventDto;

@RequiredArgsConstructor
@ApplicationScoped
public class ClientIdentityAuditController implements ClientIdentityAuditApi {
   ...
   */
  @Override
  public Response clientIdentityApiActorAudit(final String actor, final LocalDate from,
      final LocalDate to) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("access_client_identity_audit", "clientIdentity",
   ...
   */
  @Override
  public Response clientIdentityApiEntityAudit(final String uid) {
    String self = currentRequest.getActor().getName().orElse("-");
    List<AuditEvent> events = reader.findByFilters("access_client_identity_audit", "clientIdentity",
  ...
  private AuditEventDto map(final AuditEvent event) {
    return new AuditEventDto().operation(event.getOperation()).usecase(event.getUsecase())
        .entityType("clientIdentity").entityType(event.getEntityType())
