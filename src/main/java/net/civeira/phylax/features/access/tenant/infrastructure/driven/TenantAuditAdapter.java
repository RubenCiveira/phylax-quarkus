...
public class TenantAuditAdapter implements TenantAuditGateway {
  ...
  public void created(final String usecase, final Tenant tenant, final OperationContext context) {
    writer.record("_audit_access_tenant",
        AuditEvent.builder().operation("create").usecase(usecase).traceId(currentTraceId())
            ...
            .newValue(tenant.toMap()).performedBy(context.getActor().getName())
  ...
  public void deleted(final String usecase, final Tenant tenant, final OperationContext context) {
    writer.record("_audit_access_tenant",
        AuditEvent.builder().operation("delete").usecase(usecase).traceId(currentTraceId())
            .spanId(currentSpanId()).entityType("tenant").entityId(tenant.getUid())
            .oldValue(tenant.toMap()).performedBy(context.getActor().getName())
            .tenant(context.getActor().getTenant()).timestamp(context.getSource().getStartTime())
            .sourceRequest(context.getSource().getRequestId())
            .remoteAddress(context.getSource().getIp())
            .remoteApplication(context.getSource().getApplication())
