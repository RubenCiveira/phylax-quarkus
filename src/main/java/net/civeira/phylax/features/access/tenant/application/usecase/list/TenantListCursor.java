package net.civeira.phylax.features.access.tenant.application.usecase.list;

import java.util.List;
import java.util.Optional;

import lombok.Builder;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.With;

@Data
@Builder(toBuilder = true)
@With
@EqualsAndHashCode(onlyExplicitlyIncluded = true)
public class TenantListCursor {

  /**
   * max number of result
   *
   * @autogenerated ListCursorGenerator
   */
  private Integer limit;

  /**
   * @autogenerated ListCursorGenerator
   */
  private List<TenantListOrder> order;

  /**
   * @autogenerated ListCursorGenerator
   */
  private String sinceDomain;

  /**
   * @autogenerated ListCursorGenerator
   */
  private String sinceName;

  /**
   * If these value is set start retrieving for those whos uid is greater than these value
   *
   * @autogenerated ListCursorGenerator
   */
  private String sinceUid;

  /**
   * @autogenerated ListCursorGenerator
   * @return
   */
  public Optional<Integer> getLimit() {
    return Optional.ofNullable(limit);
  }

  /**
   * @autogenerated ListCursorGenerator
   * @return
   */
  public List<TenantListOrder> getOrder() {
    return null == order ? List.of() : List.copyOf(order);
  }

  /**
   * @autogenerated ListCursorGenerator
   * @return
   */
  public Optional<String> getSinceDomain() {
    return Optional.ofNullable(sinceDomain);
  }

  /**
   * @autogenerated ListCursorGenerator
   * @return
   */
  public Optional<String> getSinceName() {
    return Optional.ofNullable(sinceName);
  }

  /**
   * @autogenerated ListCursorGenerator
   * @return
   */
  public Optional<String> getSinceUid() {
    return Optional.ofNullable(sinceUid);
  }
}
