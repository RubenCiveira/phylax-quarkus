...
public class OidcUrlBuilder {
   ...
   * correct authorization flow. A {@code recovercode=} parameter is left intentionally empty at the
  ...
  public String recoverCallbackUrl(StepInput input, String username) {
    return issuer(input.tenant()) + "/recover" + "?username=" + encode(username) + "&client_id="
        + encode(input.getClientDetails().getClientId()) + input.getRequest().encodeInUrl()
        + "&recovercode=";
  }
   ...
   * correct authorization flow. A {@code regcode=} parameter is left intentionally empty at the end
   * so the caller can append the one-time code.
   *
   * @param input the current step context
  ...
  public String registerCallbackUrl(StepInput input, String email) {
    return issuer(input.tenant()) + "/register" + "?email=" + encode(email) + "&client_id="
        + encode(input.getClientDetails().getClientId()) + input.getRequest().encodeInUrl()
        + "&regcode=";
  }
