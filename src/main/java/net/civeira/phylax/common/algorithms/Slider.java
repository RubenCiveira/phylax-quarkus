...
public abstract class Slider<T> {
  ...
  private Iterator<T> applySlide(Predicate<T> predicate) {
    ...
    return new SlideIterator<>(initial, predicate, filteredData -> {
      int matchCount = filteredData.size();
      int remaining = limit - filteredData.size();
      if (remaining <= 0) {
        return null;
      }
      if (matchCount == 0) {
        // No accepted items yet; cannot estimate discard ratio. Request the full remaining limit.
        return next(filteredData, remaining);
      }
      double discardRatio = 1.0 - ((double) matchCount / limit);
      int batchSize = (int) Math.ceil(remaining / (1.0 - discardRatio));
      return next(filteredData, batchSize);
    });
  }
}
