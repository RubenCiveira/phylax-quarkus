/* @autogenerated */
package net.civeira.phylax.integrationtest.features.oauth.client;
...
public class OidcFlowClient {
  ...
  public Response submitScopeConsent(String tenant, String preSessionCookie) {
    List<String> defaults =
        Arrays.stream(OidcTestFixtures.SCOPE.split("\\s+")).filter(s -> !s.isBlank()).toList();
    return submitScopeConsent(tenant, preSessionCookie, defaults);
  }

  public Response submitScopeConsent(String tenant, String preSessionCookie,
      List<String> acceptedScopes) {
    var request = baseAuthPost(tenant).contentType(ContentType.URLENC)
        .formParam("step", "scope_consent").formParam("csid", OidcTestFixtures.CSID)
        .cookie("PRE_SESSION_ID", nullToEmpty(preSessionCookie));
    if (acceptedScopes != null && !acceptedScopes.isEmpty()) {
      request.formParam("scope_accepted", acceptedScopes.toArray(new String[0]));
    }
