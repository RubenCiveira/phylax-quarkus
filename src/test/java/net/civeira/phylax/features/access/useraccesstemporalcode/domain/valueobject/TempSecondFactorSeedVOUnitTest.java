...
class TempSecondFactorSeedVOUnitTest {
  ...
  void test_temp_second_factor_seed_v_o_builder() {
    Assertions.assertFalse(
        TempSecondFactorSeedVO.nullValue().getTempSecondFactorSeedCyphered(null).isPresent(),
        "A empty vo should have a null value");
    Assertions.assertFalse(
        TempSecondFactorSeedVO.tryFrom(null).getTempSecondFactorSeedCyphered(null).isPresent(),
    ...
    Assertions.assertThrows(ConstraintException.class,
        () -> TempSecondFactorSeedVO.tryFrom(Integer.valueOf(1)));
    ...
    Assertions.assertEquals("one", TempSecondFactorSeedVO.fromCyphered("one")
        .getTempSecondFactorSeedCyphered(null).orElse(null));
    Assertions.assertEquals("one",
        TempSecondFactorSeedVO.fromPlain("one").getTempSecondFactorSeedPlain(null).orElse(null));
    Assertions.assertEquals("plain", TempSecondFactorSeedVO.fromCyphered("one")
        .getTempSecondFactorSeedPlain(cypher).orElse(null));
    Assertions.assertEquals("cyphered", TempSecondFactorSeedVO.fromPlain("one")
        .getTempSecondFactorSeedCyphered(cypher).orElse(null));
    Assertions.assertThrows(IllegalStateException.class, () -> TempSecondFactorSeedVO
        .fromCyphered("two").getTempSecondFactorSeedPlain(cypher).orElse(null));
    Assertions.assertThrows(ConstraintException.class,
