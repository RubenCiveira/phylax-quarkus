...
class RoleChangeSetUnitTest {
    ...
    void test_role_change_set_builder() {
        RoleChangeSet one = new RoleChangeSet().uid("one").name("one").relyingParty(RelyingPartyReference.of("one")).version(1).newUid();
        one.uid("two");
        ...
        Assertions.assertEquals(RelyingPartyReference.of("one"), one.getRelyingParty().get(), "Value must equals to the intial value");
        one.relyingParty(RelyingPartyReference.of("two"));
        Assertions.assertEquals(RelyingPartyReference.of("two"), one.getRelyingParty().get(), "Value must equals to the assigned value");
        one.unset("relyingParty");
        Assertions.assertFalse(one.getRelyingParty().isPresent(), "After unset, the valeue must be dissapear");
        ...
        one.unset("version");
        Assertions.assertFalse(one.getVersion().isPresent(), "After unset, the valeue must be dissapear");
    }
}
