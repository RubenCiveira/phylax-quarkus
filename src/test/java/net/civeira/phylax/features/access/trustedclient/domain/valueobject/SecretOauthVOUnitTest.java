...
class SecretOauthVOUnitTest {
  ...
  void test_secret_oauth_v_o_builder() {
    Assertions.assertFalse(SecretOauthVO.nullValue().getCypheredValue(null).isPresent(),
        "A empty vo should have a null value");
    Assertions.assertFalse(SecretOauthVO.tryFrom(null).getCypheredValue(null).isPresent(),
    ...
    Assertions.assertEquals("one",
        SecretOauthVO.fromCyphered("one").getCypheredValue(null).orElse(null));
    Assertions.assertEquals("one", SecretOauthVO.fromPlain("one").getPlainValue(null).orElse(null));
    Assertions.assertEquals("plain",
        SecretOauthVO.fromCyphered("one").getPlainValue(cypher).orElse(null));
    Assertions.assertEquals("cyphered",
        SecretOauthVO.fromPlain("one").getCypheredValue(cypher).orElse(null));
    Assertions.assertThrows(IllegalStateException.class,
        () -> SecretOauthVO.fromCyphered("two").getPlainValue(cypher).orElse(null));
