...
class TenantVOUnitTest {
   ...
   */
  @Test
  @DisplayName("Test value object contruction for property tenant of tenant login provider ")
  void test_tenant_v_o_builder() {
    ConstraintFailList fails = new ConstraintFailList();
    Assertions.assertEquals("10", TenantVO.fromReference("10").getTenantUid(),
        "The builder must store the reference value");
    Assertions.assertEquals("11", TenantVO.tryFromReference("11").getTenantUid(),
        "The builder must store the reference value");
    Assertions.assertEquals("12", TenantVO.tryFromReference("12", fails).getTenantUid(),
        "The builder must store the reference value");
    ...
    Assertions.assertEquals(TenantReference.of("one"),
        TenantVO.from(TenantReference.of("one")).getTenant());
