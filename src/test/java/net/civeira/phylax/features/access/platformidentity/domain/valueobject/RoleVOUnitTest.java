...
class RoleVOUnitTest {
   ...
   */
  @Test
  @DisplayName("Test value object contruction for property role of platform identity role ")
  void test_role_v_o_builder() {
    ConstraintFailList fails = new ConstraintFailList();
    Assertions.assertEquals("10", RoleVO.fromReference("10").getRoleUid(),
        "The builder must store the reference value");
    Assertions.assertEquals("11", RoleVO.tryFromReference("11").getRoleUid(),
        "The builder must store the reference value");
    Assertions.assertEquals("12", RoleVO.tryFromReference("12", fails).getRoleUid(),
        "The builder must store the reference value");
    ...
    Assertions.assertEquals(RoleReference.of("one"),
        RoleVO.from(RoleReference.of("one")).getRole());
