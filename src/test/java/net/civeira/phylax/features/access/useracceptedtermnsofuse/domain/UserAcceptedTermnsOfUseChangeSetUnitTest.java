...
class UserAcceptedTermnsOfUseChangeSetUnitTest {
  ...
  void test_user_accepted_termns_of_use_change_set_builder() {
    UserAcceptedTermnsOfUseChangeSet one =
        UserAcceptedTermnsOfUseChangeSet.builder().uid("one").user(UserReference.of("one"))
    ...
    Assertions.assertEquals(TenantTermsOfUseReference.of("one"),
        one.getConditions().get().getConditions(), "Value must equals to the intial value");
    one.setConditions(TenantTermsOfUseReference.of("two"));
    Assertions.assertEquals(TenantTermsOfUseReference.of("two"),
        one.getConditions().get().getConditions(), "Value must equals to the assigned value");
    ...
    Assertions.assertEquals(
        ...
        one.getAcceptDate().get().getAcceptDate().get(), "Value must equals to the intial value");
    one.setAcceptDate(
        LocalDateTime.of(1981, 9, 6, 0, 0).atZone(ZoneId.of("Europe/Madrid")).toOffsetDateTime());
    Assertions.assertEquals(
        LocalDateTime.of(1981, 9, 6, 0, 0).atZone(ZoneId.of("Europe/Madrid")).toOffsetDateTime(),
        one.getAcceptDate().get().getAcceptDate().get(), "Value must equals to the assigned value");
