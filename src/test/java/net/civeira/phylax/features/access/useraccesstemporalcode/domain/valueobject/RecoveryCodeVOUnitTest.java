...
class RecoveryCodeVOUnitTest {
  ...
  void test_recovery_code_v_o_builder() {
    Assertions.assertFalse(RecoveryCodeVO.nullValue().getValue().isPresent(),
        "A empty vo should have a null value");
    Assertions.assertFalse(RecoveryCodeVO.tryFrom(null).getValue().isPresent(),
        "With a null value, no present result");
    Assertions.assertEquals("one", RecoveryCodeVO.from("one").getValue().orElse(null));
